...
pub fn upsert_app_usage(
    ...
    conn.execute(
        "
        INSERT INTO app_usage (date, app_name, window_title, active_seconds, first_seen_at, last_seen_at)
        VALUES (?1, ?2, ?3, ?4, ?5, ?6)
        ON CONFLICT(rowid) DO NOTHING
        ",
        params![date, app_name, window_title, seconds, first_seen, last_seen],
    )?;
    Ok(())
}
...
pub fn insert_app_usage(
    ...
    conn.execute(
        "INSERT INTO app_usage
            (date, app_name, window_title, active_seconds, first_seen_at, last_seen_at)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
        params![date, app_name, window_title, seconds, first_seen, last_seen],
    )?;
...
pub fn get_widget_config(conn: &Connection, id: &str) -> Result<Option<crate::models::WidgetConfig>> {
    let mut stmt = conn.prepare(
        "SELECT id, widget_type, x, y, width, height, opacity, always_on_top_mode, pinned
...
pub fn upsert_widget_config(conn: &Connection, cfg: &crate::models::WidgetConfig) -> Result<()> {
    conn.execute(
        "INSERT INTO widget_configs
            (id, widget_type, x, y, width, height, opacity, always_on_top_mode, pinned)
